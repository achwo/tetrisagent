...
def reward(s, a):
    ...
    if row > len(s) - 2:
        return (final_score(s), None)

    new_s = create_new_state(s, row, a)
    ...
    if row == len(new_s) - 2:
        if len(consecutive_zeros) <= 0 or max(consecutive_zeros) < 2:
            return (final_score(new_s), new_s)
...
def play(episodes, training, width, height, start_epsilon, start_alpha, Q_in,
    ...
    while episodes is not None and episodes_played < episodes or \
        ...
        last_s, last_score = episode(S0, Q, A, epsilon, alpha, interactive)
        episodes_played += 1
