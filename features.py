...
def max_height(self):
    ...
    for i in environment.blocks:
        for j in environment.blocks[count_i]:
            if environment.blocks[count_i][count_j] is not 0 and (
                FIELD_HEIGHT - count_j) > max_height:
                max_height = FIELD_HEIGHT - count_j
                break
            elif environment.blocks[count_i][count_j] is not 0:
                break
            count_j += 1
        count_i += 1
    return max_height
...
def individual_height(self):
    ...
    for i in environment.blocks:
        for j in environment.blocks[count_i]:
            if environment.blocks[count_i][count_j] is not 0:
...
def number_of_holes(environment):
    ...
    for col in environment.blocks:
        possible_hole = False
        for field in col:
            if possible_hole and field is 0:
                holes += 1
            elif field is not 0:
                possible_hole = True
    return holes
