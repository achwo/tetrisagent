...
class TemporalDifferenceLearningWithEpsilonGreedyPolicy(Algorithm):
    ...
    def choose_action(self, Q, ACTIONS, state):
        ...
        candidates = set(random.sample(ACTIONS, 1))
        for action in ACTIONS:
            curr_candidate = next(iter(candidates))
            #print("Q({}, {}): {}".format(state, action, Q[(state, action)]))
            if Q[(state, action)] > Q[(state, curr_candidate)]:
                candidates = {action}
            elif Q[(state, action)] == Q[(state, curr_candidate)]:
    ...
    def episode(self, S0, Q, ACTIONS, epsilon, alpha, interactive):
        ...
        state = S0
        last_field = state
        while not self.world.is_final_state(state):
            ...
            reward, next_state = self.world.execute_action(state, action)
